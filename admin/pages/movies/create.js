...
export default function CreateMovie() {
  ...
  const handlePlatformChange = (platform) => {
    const platforms = formData.streamingPlatforms.includes(platform)
      ? formData.streamingPlatforms.filter(p => p !== platform)
      : [...formData.streamingPlatforms, platform];
    setFormData({ ...formData, streamingPlatforms: platforms });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!posterFile && !formData.posterUrl) {
      toast.error('Please upload a poster image or provide a poster URL');
      return;
    }
  ...
  return (
    <Layout title="Create Movie">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Create New Movie</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
              ...
              <div className="flex-1 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Enter image URL (faster and easier)
                  </p>
                </div>
          ...
          {/* Streaming Platforms */}
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Streaming Platforms</h2>
            <p className="text-sm text-gray-600 mb-4">Select which platforms this {formData.type === 'series' ? 'series' : 'movie'} is available on</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { id: 'netflix', name: 'Netflix', color: 'red' },
                { id: 'prime', name: 'Amazon Prime', color: 'blue' },
                { id: 'disney', name: 'Disney+ Hotstar', color: 'indigo' },
                { id: 'apple', name: 'Apple TV+', color: 'gray' },
                { id: 'hotstar', name: 'Hotstar', color: 'yellow' },
                { id: 'zee5', name: 'Zee5', color: 'purple' },
                { id: 'sonyliv', name: 'SonyLIV', color: 'green' },
                { id: 'youtube', name: 'YouTube', color: 'red' },
              ].map(platform => (
                <label key={platform.id} className="flex items-center space-x-3 p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors" style={{
                  borderColor: formData.streamingPlatforms.includes(platform.id) ? `var(--color-${platform.color}-500)` : '#d1d5db'
                }}>
                  <input
                    type="checkbox"
                    checked={formData.streamingPlatforms.includes(platform.id)}
            ...
            <p className="text-xs text-gray-500 mt-3">
              ✓ Only selected platforms will show in the "Where to Watch" section
            </p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Streaming & Purchase Links</h2>
              <button
                type="button"
                ...
                <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 border border-gray-300 rounded-lg">
                  <div className="md:col-span-3">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                    >
                      <option value="iTunes HD">iTunes HD</option>
                      <option value="iTunes 4K">iTunes 4K</option>
                      <option value="Google Play HD">Google Play HD</option>
                      <option value="YouTube Movies HD">YouTube Movies HD</option>
                      <option value="Amazon Buy HD">Amazon Buy HD</option>
                      <option value="Amazon Rent HD">Amazon Rent HD</option>
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Price (or leave empty for streaming)
                    </label>
                    <input
                      type="text"
                      value={link.size}
                      ...
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                      placeholder="$3.99 or Free with subscription"
                    />
                  </div>
                  <div className="md:col-span-6">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      URL (iTunes, Google Play, YouTube, Amazon, etc.)
                    </label>
                    <input
                      type="url"
                      value={link.url}
                      ...
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                      placeholder="https://tv.apple.com/movie/... or https://play.google.com/..."
                      required
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      ✓ Use official platform links only (iTunes, Google Play, YouTube Movies, Amazon)
                    </p>
                  </div>
